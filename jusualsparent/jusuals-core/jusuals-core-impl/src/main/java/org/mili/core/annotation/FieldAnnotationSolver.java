...
/**
 * This interface defines a field annotation solver.
 *
 * @author Michael Lieshoff
 */
public class FieldAnnotationSolver extends AbstractAnnotationSolver<Method>{
    @Override
    ...
    private void addAnnotations(Class<?> cls, List<WrappedAnnotation> annotations) {
        for(Field field : cls.getDeclaredFields()) {
            this.addFieldAnnotations(field, annotations);
        }
    }

    private void addFieldAnnotations(Field source, List<WrappedAnnotation> annotations) {
        for(Annotation annotation : source.getAnnotations()) {
            annotations.add(new WrappedAnnotation<Field>(annotation, source));
        }
    }
}
